...
class MeasureComponent(object):
    def __init__(self, row):
        ...
        self.monetary_unit_code = row[3] if row[3] is not None else ""
        self.measurement_unit_code = row[4] if row[4] is not None else ""
        self.measurement_unit_qualifier_code = row[5] if row[5] is not None else ""
        ...
        self.get_cts_component_definition()
        self.get_english_component_definition()

    def process_measure_component(self):
        self.duty_string = ""
        if self.monetary_unit_code == "":
            self.duty_string = "specific"
        else:
            self.duty_string = "advalorem"
